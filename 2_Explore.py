...
ax3 = ax3.flatten()  # Flatten the 2D array of axes to make it easier to iterate over. It avoids the use of nested loop.

# Loop through each numeric column and create a boxplot
for i, col in enumerate(numeric_df.columns):
    sns.boxplot(y=numeric_df[col], ax=ax3[i])
    ax3[i].set_title(f"{col.split('_')[0].upper() + ' ' + col.split('_')[1].upper()}")
    ax3[i].set_ylabel('')
...
sns.histplot(x=df['release_year'], ax=ax4, binwidth=1, color='steelblue', edgecolor='white')
...
sns.countplot(x=df['console'], ax=ax5, color='#c86f7f', edgecolor='white',
...
pc_highlight_palette = np.where(top15_consoles.index == 'PC', '#c86f7f', 'steelblue')
fig6, ax6 = plt.subplots(figsize=(12, 8))
sns.barplot(y=top15_consoles, x=top15_consoles.index, palette=pc_highlight_palette, edgecolor='white')
plt.xticks(rotation=90)
...
sns.countplot(x=df['genre'], ax=ax7, color='steelblue', edgecolor='white',
...
# Okay then, we have a lot of information yet. We can see that we have a lot of missing values.
# It will be a bit problematic but not yet. We see that PC are overrepresented in the dataset, and most games recorded
# where released in 2009. We show also that even if PC are the support with the most released games, it's only at
# the 12th position of sales by support.
